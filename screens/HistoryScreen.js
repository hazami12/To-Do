...
import { getToDoDays } from "../data/firestopreRealTime";
...
const NewCalendarListScreen = () => {
  ...
  useEffect(() => {
    getFormattedToDoDays(setMarkedDates, setLoaded);
  }, []);
...
function getFormattedToDoDays(setMarkedDates, setLoaded) {
  const ToDoDays = getToDoDays();
  let formattedData = {};
  setTimeout(function () {
    ToDoDays.forEach((date) => {
      formattedData[date] = {
        selected: true,
        // marked: true,
